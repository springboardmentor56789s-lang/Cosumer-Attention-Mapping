...
def dispatch_otp(target: str, code: str = None, purpose: str = "authentication", channel: str = None) -> dict:
    """
    Generate and send 6-digit OTP code to email or phone, storing it in SQLite database.
    """
    ...
    # 1. Record OTP entry in Database
    db_saved = False
    try:
        db = SessionLocal()
    ...
    # 2. Dispatch via Email (SMTP) or Phone SMS (Twilio / Fast2SMS)
    if is_email:
...
def main():
    ...
    if not targets:
        ...
        print("Examples:")
        print("  python send_otp.py --email eleanor@retail.com")
        print("  python send_otp.py --phone +15550192834")
        print("  python send_otp.py --email eleanor@retail.com --phone +15550192834")
        sys.exit(1)
