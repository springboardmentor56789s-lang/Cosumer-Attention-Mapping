...
export default function LandingPage() {
  const { loginManager, loginWorker, loading } = useAuth();
  ...
  const handleQuickManagerDemo = async () => {
    const res = await loginManager('eleanor@retail.com', 'password123');
    if (res.success) {
      navigate('/manager-dashboard');
    }
  };

  const handleQuickWorkerDemo = async () => {
    const res = await loginWorker('sarah.worker@retailstore.com', 'password123');
    if (res.success) {
        ...
        <div className="lg:col-span-5 max-w-md w-full mx-auto space-y-7 pr-0 lg:pr-2">
          ...
          <div className="space-y-3">
            ...
            <p className="text-xs text-slate-400 leading-relaxed">
              Spatial Retail Intelligence & Store Operations System. Select your entry mode.
            </p>
          </div>
          ...
          {/* Role Direct Entry Buttons */}
          <div className="space-y-2.5">
            <Link
              to="/auth/manager/login"
              className="w-full p-3.5 bg-[#17171c] hover:bg-[#202027] border border-purple-500/30 hover:border-purple-500/60 rounded-full text-xs font-semibold text-white flex items-center justify-between transition group shadow-md"
            >
              <div className="flex items-center gap-2.5">
                <ShieldCheck className="w-4 h-4 text-purple-400" />
                <span>Continue as Store Manager</span>
              </div>
            ...
            <Link
              ...
              className="w-full p-3.5 bg-[#17171c] hover:bg-[#202027] border border-blue-500/30 hover:border-blue-500/60 rounded-full text-xs font-semibold text-white flex items-center justify-between transition group shadow-md"
            >
              <div className="flex items-center gap-2.5">
                <HardHat className="w-4 h-4 text-blue-400" />
                <span>Continue as Store Worker</span>
              </div>
          ...
          <div className="space-y-3 pt-1 text-xs">
            ...
            <button
              ...
              disabled={loading}
              className="w-full py-3.5 bg-[#e2e8f0] hover:bg-white text-slate-950 font-bold text-xs rounded-full shadow-lg transition duration-200 flex items-center justify-center gap-2"
            >
              <span>Continue with Demo Manager Account</span>
            </button>
          </div>
