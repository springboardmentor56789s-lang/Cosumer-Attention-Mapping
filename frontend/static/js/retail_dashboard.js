...
document.addEventListener("DOMContentLoaded", () => {
    ...
    async function loadInsights() {
        ...
        const products = Array.isArray(series.product_performance)
            ? series.product_performance.filter((row) => row)
            : [];
        const insightTable = document.getElementById("insightTable");
        if (insightTable) insightTable.innerHTML = products.map((row) => `
            <tr class="border-b hover:bg-slate-50">
                <td class="p-3 font-medium">${row.name}</td>
                <td class="p-3">${Number(row.views || 0).toLocaleString()}</td>
                <td class="p-3">${Number(row.attention || 0).toFixed(1)}%</td>
            </tr>
        `).join("") || `<tr><td colspan="3" class="p-3">No product analytics yet</td></tr>`;
        ...
        chart = null;
        const hourly = Array.isArray(series.hourly_visitors)
